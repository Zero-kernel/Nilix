...
calculate_regression() {
    ...
    if [ "$measured" -eq 0 ]; then
        echo "PERF-CASE name=$name status=deferred owner=scripts/gates/performance/perf_regression_test.sh reason=measurement-workload-unimplemented"
        return 3
    fi
