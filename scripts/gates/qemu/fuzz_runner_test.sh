...
TEST_TIMEOUT="${KERNEL_TEST_TIMEOUT:-900}"

if ! [[ "$TEST_TIMEOUT" =~ ^[1-9][0-9]*$ ]] ||
   [ "$TEST_TIMEOUT" -lt 5 ] || [ "$TEST_TIMEOUT" -gt 900 ]; then
    echo "KCOV-E2E BLOCKED: KERNEL_TEST_TIMEOUT must be an integer from 5 to 900"
    exit 2
fi
