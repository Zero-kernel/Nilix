...
import gate_log_test as gate_fixture
import stress_protocol
...
class StressOutcomeTests(unittest.TestCase):
    def test_profile_and_aggregate_statuses(self):
        source = (ROOT / "scripts" / "stress_test.sh").read_text(encoding="utf-8")
...
class OtherCallerOutcomeTests(unittest.TestCase):
    def setUp(self):
        ...
        shutil.copyfile(
            ROOT / "scripts" / "perf_regression_test.sh",
            self.fixture.work / "scripts" / "perf_regression_test.sh",
        )
...
class RecordedGateTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("record_gate.py", "qemu_fuzz_smoke.py"):
