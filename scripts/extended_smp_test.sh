...
if [[ ! "$EXTENDED_SMP_TIMEOUT" =~ ^([1-9]|[1-9][0-9]|1[01][0-9]|120)$ ]]; then
    echo "EXTENDED-SMP-TEST BLOCKED: EXTENDED_SMP_TIMEOUT must be an integer from 1 to 120"
    exit 2
fi
...
run_smp_test() {
    ...
    local result=$?
    python3 "$ROOT/scripts/gate_inputs.py" verify --output "$ser.inputs" || result=1
...
echo ""
total_tests=$((total_tests + 1))
run_smp_test 16 "$((EXTENDED_SMP_TIMEOUT + 30))"
record_result "$?"
