...
class CommandTests(unittest.TestCase):
    def test_cli_exit_codes_and_missing_evidence(self):
        with tempfile.TemporaryDirectory(prefix="gate-cli-") as directory:
            ...
            command = [sys.executable, str(ROOT / "scripts" / "gate_log.py")]
...
class ShellTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for name in (*self.scripts, "gate_log.py", "gate_inputs.py", "musl_check.sh"):
...
""")
    ...
    def run_gate(
        ...
        result = subprocess.run(
            [shutil.which("bash"), "--noprofile", "--norc", self.shell_path(self.work / "scripts" / name)],
