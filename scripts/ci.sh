...
set -euo pipefail
cd "$(dirname "$0")/.."
group="${1:?usage: bash scripts/ci.sh quality|hosted|tools|build|runtime|musl|iommu|mitigation|qemu-fuzz|extended}"
...
finish() {
    local status=$?
    trap - EXIT
    if python3 scripts/ci_report.py "$out"; then
...
gate() {
    local name="$1"
    shift
    python3 scripts/record_gate.py --name "$name" --artifacts "$out/$name" "$@"
}
check() {
...
case "$group" in
    quality)
        ...
        check python -- python3 -m pytest scripts -q --durations=15 \
        ...
        check shell-syntax -- bash -c 'for script in scripts/*.sh .githooks/*; do bash -n "$script" || exit; done'
        ;;
    hosted)
    ...
    runtime)
        ...
        check boot --allow-qualified -- bash scripts/boot_check.sh esp
        check runtime --allow-qualified -- bash scripts/kernel_test.sh esp
        check smp --allow-qualified -- bash scripts/smp_test_4core.sh esp
        ;;
    musl)
...
PY
        ...
        check activation --allow-qualified -- bash scripts/iommu_q35_check.sh esp
        check failures -- python3 scripts/iommu_failure_probe.py --input-manifest "$out/inputs.sha256" --revision "$revision" --artifacts "$out/failure-probes"
    ...
    qemu-fuzz)
        ...
        check kcov-guest -- env KERNEL_TEST_TIMEOUT=120 KCOV_KEEP_LOGS=1 make test-kcov
        gate executor-build -- make build-fuzz-qemu-deps
        check executor-guest -- python3 scripts/qemu_fuzz_smoke.py --kernel esp-syz/kernel.elf \
            --artifacts "$out/executor-evidence"
        ;;
    extended)
        gate build -- make build
        check smp-8-16 --allow-qualified -- bash scripts/extended_smp_test.sh esp
        check ext3 --allow-qualified -- bash -c '
            ...
            KERNEL_TEST_DISK="$image" bash scripts/kernel_test.sh esp
        '
        gate stress-build -- make build-stress
        check stress --allow-qualified -- env STRESS_DURATION="${CI_STRESS_SECONDS:-60}" STRESS_CPUS=4 \
            STRESS_KEEP_ARTIFACTS=1 bash scripts/stress_test.sh esp-stress
        ;;
