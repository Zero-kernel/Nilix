...
def main():
    ...
    parser.add_argument("--timeout", type=int, default=90,
                        help="total boot, tracing and workload deadline in seconds (1..600)")
    parser.add_argument("--smp", type=int, choices=(4,), default=4)
    parser.add_argument("--cpu", default="qemu64,+smep,+smap,+umip,+rdrand")
    args = parser.parse_args()
    if not 1 <= args.timeout <= 600 or (args.input_manifest and not args.revision):
        parser.error("timeout must be 1..600; input manifest requires revision")
