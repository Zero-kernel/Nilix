...
impl MockKernelContext {
    ...
    /// Get process count (for coverage feedback)
    pub fn process_count(&self) -> usize {
        self.processes.len()
    }

    /// Get FD count (for coverage feedback)
    pub fn fd_count(&self) -> usize {
        self.processes.values()
            .map(|p| p.open_fds.len())
            .sum()
    }
    ...
    // Syscall handlers - minimal implementations that exercise validation logic
