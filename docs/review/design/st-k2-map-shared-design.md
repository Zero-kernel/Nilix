...
**Status:** Current-tree revalidation required (2026-09-12); historical September 1 design was READY FOR IMPLEMENTATION. Phase 2 retains its §7/§4.1 premise checks.
...
**Phase 1:** allowlist arm after the LSM hook: accept `MAP_PRIVATE|MAP_ANONYMOUS` (+hint
addr); EOPNOTSUPP otherwise (incl. MAP_FIXED, MAP_SHARED; unknown bits). Guest:
`run_in_cgroup_child` → `pipe()` write-once/read-exact with EINTR retry; `shared_mmap`
fail-closes honestly (`errno=95`) until Phase 2.
