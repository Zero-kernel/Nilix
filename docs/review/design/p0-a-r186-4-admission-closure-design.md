...
**Status:** Current-tree revalidation required (2026-09-12); historical September 1 design was READY FOR IMPLEMENTATION.
**Current handoff:** [source refresh](next-handoff-2026-09-12.md#3-p0-a--r186-4--current-allocation-and-ownership-premises) preserves the accepted mechanism/charge decisions and reconciles the later KSA cleanup paths. Current implementation acceptance requires independent mechanism review and final source-bound tests.
...
- `struct MmState` opens at `kernel_core/process.rs:1179`;
  ...
  `from_sorted_vec_charged` with `shrink_to_fit` at `mm/admitted.rs:1011` — the shrink
...
- **Tests today (registered names, as the harness reports them):**
...
- **D1 — fork pre-admission allocation.** `fork.rs:628-631` builds the snapshot Vec with a
  ...
  NOT unwind double-panic — §2's framing corrected); constructor Err → reservation still
  owned by caller, dropped explicitly; later fork failures (charge/PT-clone) → charge owned
...
0. **Defect repairs (D1-D3, HIGH floor):**
   - `kernel/kernel_core/fork.rs:628-631` — admission reservation BEFORE the snapshot Vec
     allocation (`// P0-A FIX:` marker); release/fold on the from_sorted_vec_charged handoff.
   - `kernel/mm/admitted.rs:1004-1020` — remove `shrink_to_fit`; charge on pre-shrink
     `entries.capacity()` (over-count-safe) or fallible reserve+move; no infallible realloc.
   - `kernel/src/runtime_tests.rs` — rewrite all three tests' oracles on
     `heap_class_snapshot(CoreProcess)` live deltas (CAPACITY model); `:561` Warning→Fail;
     size the pressure leg past the 512 KiB cap; fix the inverted reclamation oracle
     (assert reclaim-on-empty, retained-capacity-on-partial).
...
3. Remote ladder: `make build && make lint && make test` (0 failed, three tests in passed
   set), `make boot-check`, `make musl-check`. Exit codes captured explicitly.
4. Plan status sync: R186-4 → CLOSED; D1-RES-HEAP-ADMISSION-REOPENED → RESOLVED; gate table
   row 1 recomputed (stage 5 records; stage 4 next rollover re-ranks).
...
| Remote ladder | build/lint/test/boot/musl PASS with explicit exit codes |

## 9. Open Questions

None blocking. (Multi-CPU interleave breadth explicitly remains carried P2 debt, not this slice.)
...
`runtime_tests.rs:223/303/422` · R186-4 spec (qa-2026-07-28.md) · U23-1/U23-2
