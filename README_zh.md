...
Zero-OS 是一个企业级混合内核，灵感来自 Linux 的模块化设计，并经过 **172 轮持续安全审计**
...
| CI 与质量门禁                  | ✅ 完成     | GitHub Actions（fmt/clippy、build、lint、boot+musl）、自定义 lint 门禁、远程 pre-push 钩子      |
...
├── .githooks/pre-push      # 远程 fmt + clippy 门禁
...
Zero-OS 自动强制执行正确性、风格与启动健康度。所有构建/测试命令都在 Linux 构建主机上运行
（本地检出仅为只读镜像）。
...
- **`.githooks/pre-push`** —— 选择性启用（`git config core.hooksPath .githooks`）。由于本地
  Windows 镜像没有工具链，钩子会 SSH 到构建主机，在镜像的工作树上于每次 push 前运行
  `make fmt-check` + `make clippy`。用 `SKIP_PREPUSH=1 git push` 可跳过单次 push。

---

## 6. 安全审计状态

Zero-OS 在持续的对抗式评审流程下开发：每一轮审计内核、按严重程度记录发现、修复它们，并在
...
1. 推送前运行 `make build`、`make lint`、`make boot-check`，并（对 ABI 改动）运行
   `make musl-check`；pre-push 钩子还会运行 `fmt-check` + `clippy`。
