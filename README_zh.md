...
**最近新增：**
- **2026-07-29：** R186 review-fix 复核轮次 —— 针对已落地的 R186 修复，重新审查其所在的锁上下文、
  完整调用方集合与回滚路径，**记录并修复了 10 项 review-fix 缺陷（`RF186-1`…`RF186-12`）**。
  其中两项为彻底失败：R186-1 的修复只是把 open/openat 对当前 PCB 的递归加锁**换了个位置**而未消除；
  R186-10 引入的 COW 重试预算会让正常的跨 CPU 页表竞争变成用户致命错误（现改为依据 PT 锁持有者身份判定，
  仅真正的同 CPU 重入才致命）。本轮还为设备 MMIO 引入带类型的 BAR 孔径授权、全有或全无的映射事务与
  分阶段 MSE/BME 激活；在“仲裁 → 铸造 → 发布 → 审计”的完整跨度内保持同一凭证身份；使 `cgroupfs`
  的 `readdir` 完全无分配；将 ext2 目录项类型绑定到权威 inode mode；并把 `CapTable` 封装到
  `#[must_use]` 的两阶段受仲裁铸造接口之后。所有 CI 门禁通过，启动测试套件实测
  **31 通过 / 39 延后 / 0 失败**。
...
| 指标                 | 数值                                     |
| -------------------- | ---------------------------------------- |
| 审计轮次             | **186**（R186 于 2026-07-28 完成；review-fix 复核 2026-07-29） |
| 累计发现             | ~1,333（历史 ID 含合并/驳回项）          |
| 已修复/解决的发现    | ~1,177                                   |
| 最新轮次             | R186 —— 16 完全修复、1 可执行项开放、1 INFO |
| 最新 review-fix 复核 | RF186 —— 在 R186 的修复中记录并修复 10 项缺陷 |
| 当前可执行债务       | **1 HIGH**（`R186-4`）                    |
| 1.0-Preview 发布门禁 | **已阻塞** —— 尚余一个 HIGH；零 HIGH 连续记录 0/3 |

R186 共记录 18 个问题：17 个可执行发现与 1 个 INFO。16 个可执行项已完全修复，其中
`R186-18` 通过由共享凭证对象持有代际计数器并在两条 open 路径中校验而关闭；`R186-4` 是唯一
仍开放的 HIGH。因此零 HIGH 连续记录重置为 0/3，聚合堆准入的设计父项也被重新打开。精确状态见
[R186 报告](docs/review/audits/qa-2026-07-28.md)，剩余工作见
[当前计划](docs/review/nextplan/next-phase-plan-2026-07-23-v2.md)。

随后的 **RF186 review-fix 复核（2026-07-29）** 审查的对象是这些修复本身、而非原始发现，结论是其中
10 项只关闭了报告中的个例、未关闭整个缺陷类别 —— 2 项彻底失败（`RF186-1`、`RF186-6`），8 项部分成立。
10 项均已修复；本轮未触及 `R186-4`，因此连续记录仍为 0/3（review-fix 复核不计入连续记录）。
本轮以 **MODE S** 执行 —— Codex MCP 同行不可达，收敛仅依赖编排方对 `file:line` 的复读而非独立会话，
因此 `RF186-4`/`RF186-7` 已标记为需由下一轮审计从源码重新推导。本轮同时记录了**测试债务**：
新增于 `seccomp`/`net`/`block`/`kernel_core` 的 `#[cfg(test)]` 断言实际上不会在任何地方执行 ——
既没有任何目标运行 `cargo test`，这些 crate 的宿主测试二进制也会因内核堆未初始化而中止
（此为既有状况，已 A/B 验证）。详见
[RF186 报告](docs/review/reviewfix/reviewfix-2026-07-29.md)。
